...
class CorrContext:
    ...
    masks_pyramid: Optional[List[torch.Tensor]] = None
    ...
    def time_slice(self, start: int, end: int) -> 'CorrContext':
        ...
        return CorrContext(
            ...
            masks_pyramid=self.masks_pyramid,  # Don't slice masks - need full sequence for KNN context
        )
...
class KNNCorrFeature4D_Optimized(nn.Module):
    ...
    def prepare_window(self, *, shared_ctx: CorrContext, feats: torch.Tensor, pcds: torch.Tensor, queries: torch.Tensor, camera_locs: torch.Tensor, projector: Callable[[torch.Tensor], torch.Tensor], masks: Optional[torch.Tensor] = None) -> CorrContext:
        ctx = shared_ctx.copy()
        assert ctx.verify_shared()
        ...
        # Build mask pyramid if masks are provided
        if masks is not None:
            # masks: (B, T_mask, H, W) binary masks
            B_mask, T_mask = masks.shape[:2]
            ctx.masks_pyramid = []
            masks_current = masks.clone()  # Start with original resolution
            for i in range(len(ctx.feat_pyramid)):
                # Downsample mask to match feature resolution at this level
                target_h, target_w = ctx.feat_pyramid[i].shape[-2:]
                if i == 0:
                    # First level: resize to feature map size
                    masks_resized = torch.nn.functional.interpolate(
                        rearrange(masks_current.float(), 'b t h w -> (b t) h w').unsqueeze(1),  # (B*T_mask, 1, H, W)
                        size=(target_h, target_w),
                        mode='nearest'
                    ).squeeze(1) > 0.5  # (B*T_mask, H, W) back to binary
                    masks_current = rearrange(masks_resized, '(b t) h w -> b t h w', b=B_mask, t=T_mask)
                else:
                    # Subsequent levels: downsample by 2x from previous level
                    prev_masks = ctx.masks_pyramid[-1]  # (B, T_mask, H_prev, W_prev)
                    masks_resized = torch.nn.functional.interpolate(
                        rearrange(prev_masks.float(), 'b t h w -> (b t) h w').unsqueeze(1),
                        size=(target_h, target_w),
                        mode='nearest'
                    ).squeeze(1) > 0.5
                    masks_current = rearrange(masks_resized, '(b t) h w -> b t h w', b=B_mask, t=T_mask)
                ctx.masks_pyramid.append(masks_current)
        else:
            # Initialize as empty list when masks are not used
            ctx.masks_pyramid = []
    ...
    def forward(self, ctx: Box, curr_coords: torch.Tensor, max_points_per_chunk: Optional[int] = 384) -> torch.Tensor:
        ...
        curr_knn_idxs = []
        original_indices_list = []  # Store original indices for each level
        with torch.autocast("cuda", enabled=False):
            ...
            for i in range(self.corr_levels):
                ...
                # Apply mask filtering if masks are provided
                if ctx.masks_pyramid is not None and len(ctx.masks_pyramid) > 0 and i < len(ctx.masks_pyramid):
                    B_val, T_val = ctx.pcd_pyramid[i].shape[0], ctx.pcd_pyramid[i].shape[1]
                    mask_level = ctx.masks_pyramid[i]  # (B, T, H, W)
                    mask_flat = rearrange(mask_level, 'b t h w -> (b t) (h w)')  # (B*T, H*W)

                    # Count valid points per batch (each batch is one B*T combination)
                    valid_counts_per_batch = mask_flat.sum(dim=1)  # (B*T,)
                    min_valid_count = valid_counts_per_batch.min().item()

                    # Fallback: if any batch has fewer than k_neighbors valid points, don't use masks
                    if min_valid_count < self.k_neighbors:
                        logger.warning(f"Level {i}: Minimum valid points ({min_valid_count}) < k_neighbors ({self.k_neighbors}). Skipping mask filtering.")
                        ...
                        # Filter context coords using mask
                        # We need to maintain batch structure for KNN query
                        filtered_coords_list = []
                        original_idx_list = []
                        batch_offsets = [0]

                        for bt in range(mask_flat.shape[0]):
                            mask_bt = mask_flat[bt]  # (H*W,)
                            valid_indices_bt = torch.where(mask_bt)[0]  # Indices of valid points
                            filtered_coords_list.append(map_pcds[bt, valid_indices_bt])  # (N_valid, 3)
                            original_idx_list.append(valid_indices_bt)
                            batch_offsets.append(batch_offsets[-1] + valid_indices_bt.shape[0])

                        context_coords = torch.cat(filtered_coords_list, dim=0)  # (Total_valid, 3)
                        original_indices = original_idx_list  # List of tensors, one per batch
                        context_batch_offsets = torch.tensor(batch_offsets[1:], device=map_pcds.device, dtype=torch.int32)
                else:
                    context_coords = map_pcds.reshape(-1, 3)
                    original_indices = None
                    context_batch_offsets = (torch.arange(map_pcds.shape[0], device=map_pcds.device, dtype=torch.int32) + 1) * map_pcds.shape[1]
                ...
                assert hw >= self.k_neighbors or original_indices is not None
            ...
            for i in range(self.corr_levels):
                ...
                knn_idx = knn_outputs[i]  # (B*T*N, K)

                # Convert filtered indices back to original map indices
                if original_indices_list[i] is not None:
                    # knn_idx contains indices into filtered context
                    # We need to map them back to original map indices
                    B_val, T_val = ctx.pcd_pyramid[i].shape[0], ctx.pcd_pyramid[i].shape[1]
                    knn_idx_reshaped = knn_idx.reshape(B_val * T_val, num_points, self.k_neighbors)

                    # Calculate batch offsets for each batch
                    batch_start_offsets = [0]
                    for bt in range(B_val * T_val):
                        batch_start_offsets.append(batch_start_offsets[-1] + len(original_indices_list[i][bt]))

                    for bt in range(B_val * T_val):
                        original_idx_bt = original_indices_list[i][bt]  # (N_valid,)
                        # Map filtered indices to original indices
                        # Subtract batch offset to get local indices within this batch's filtered context
                        knn_indices_global = knn_idx_reshaped[bt]  # (num_points, K)
                        knn_indices_local = knn_indices_global - batch_start_offsets[bt]

                        # Map local filtered indices to original map indices
                        knn_idx_reshaped[bt] = original_idx_bt[knn_indices_local]

                    curr_knn_idxs.append(knn_idx_reshaped.long())
                else:
                    curr_knn_idxs.append((knn_idx % hw).reshape(-1, num_points, self.k_neighbors).long())
