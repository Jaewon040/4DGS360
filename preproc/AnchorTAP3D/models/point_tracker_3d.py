    ...
    def _forward_window(
        ...
        *,
        ...
        for it in range(num_iters):
            ...
            # Apply 2D track guidance: force visible points to 2D track locations
            if guidance_2d is not None:
                # Use actual coords time dimension (handles padding/partial windows gracefully)
                T_actual = coords.shape[1]

                # Safely slice guidance to match coords actual size
                if T_actual <= guidance_2d['visibility'].shape[1]:
                    vis_mask = guidance_2d['visibility'][:, :T_actual] > track_vis_threshold  # [B, T_actual, N]
                    coords_from_2d = guidance_2d['coords_3d'][:, :T_actual]  # [B, T_actual, N, 3]

                    # Replace visible points with 2D track locations
                    coords = torch.where(
                        vis_mask[..., None],  # [B, T_actual, N, 1]
                        coords_from_2d,       # 2D track based coordinates
                        coords                # TAPIP3D predicted coordinates
                    )
                else:
                    # Edge case: coords is somehow larger than guidance (shouldn't happen, but be safe)
                    logger.warning(f"Coords time dimension ({T_actual}) > guidance time dimension ({guidance_2d['visibility'].shape[1]}), skipping guidance for this iteration")
    ...
    def _wrapped_forward_window(
        ...
        *,
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            # Normalize guidance_2d if provided
            normalized_guidance = None
            if guidance_2d is not None:
                if self.norm_mode != "none":
                    # Transform guidance coords to normalized space
                    normalized_guidance = {
                        'coords_3d': (guidance_2d['coords_3d'] - original_ctx.mean_coords[:, None, None, :]) / original_ctx.std_coords[:, None, None, :] * self.norm_scale,
                        'visibility': guidance_2d['visibility'],
                    }
                else:
                    normalized_guidance = guidance_2d

            with record_function("prepare_window"):
                # Slice masks to window if provided
                windowed_masks = None
                if masks is not None:
                    windowed_masks = masks[:, window_start:window_end]  # (B, window_size, H, W)
        ...
        # Slice guidance to window if provided
        windowed_guidance = None
        if normalized_guidance is not None:
            windowed_guidance = {
                'coords_3d': normalized_guidance['coords_3d'][:, window_start:window_end],
                'visibility': normalized_guidance['visibility'][:, window_start:window_end],
            }
    ...
    def streaming_forward(
        ...
        *,
        ...
        if rgb_obs.shape[1] % (self.seq_len // 2) != 0:
            ...
            # Pad guidance_2d if provided
            if guidance_2d is not None:
                guidance_2d = {
                    'coords_3d': torch.cat([
                        guidance_2d['coords_3d'],
                        guidance_2d['coords_3d'][:, -1:].expand(-1, pad, -1, -1)
                    ], dim=1),
                    'visibility': torch.cat([
                        guidance_2d['visibility'],
                        guidance_2d['visibility'][:, -1:].expand(-1, pad, -1)
                    ], dim=1),
                }
            # Pad masks if provided
            if masks is not None:
                masks = torch.cat([masks, masks[:, -1:].expand(-1, pad, -1, -1)], dim=1)
            logger.info(f"Padded sequence from {original_T} to {rgb_obs.shape[1]} frames (pad={pad}) to fit window size {self.seq_len}")
        ...
        for window_end in range(self.seq_len, T + 1, self.seq_len // 2):
            ...
            if B > 1:
                ...
                if track_mask[0].any():
                    ...
                    # Slice guidance_2d by mask if provided
                    masked_guidance = None
                    if guidance_2d is not None:
                        masked_guidance = {
                            'coords_3d': guidance_2d['coords_3d'][:, :, mask, :],
                            'visibility': guidance_2d['visibility'][:, :, mask],
                        }
