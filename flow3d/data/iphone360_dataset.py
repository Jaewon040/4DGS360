...
@dataclass
class iPhoneTapip3dDataConfig:
    data_dir: str
    data_type: str = "iphone360"
    start: int = 0
    end: int = -1
    ...
    factor: int = 1
    camera_type: Literal["tapip3d", "original", "refined"] = "refined"
    skip_load_imgs: bool = False
    val_fgonly: bool = False
    val_fgonly_bgwhite: bool = False


class iPhoneTapip3dDataset(BaseDataset):
    def __init__(
        self,
        data_dir: str,
        data_type: str = "iphone360",
        ...
        **_,
        ...
        self.skip_load_imgs = skip_load_imgs
        self.val_fgonly = val_fgonly
        self.val_fgonly_bgwhite = val_fgonly_bgwhite

        # Check if validation split exists
        with open(osp.join(data_dir, "splits", "val.json")) as f:
            val_split_dict = json.load(f)
        self.has_validation = len(val_split_dict["frame_names"]) > 0
        ...
        # Time IDs for validation
        # Use full_len for the default range, not len(self.frame_names) which is already sliced
        time_ids = [t for t in split_dict.get("time_ids", list(range(full_len)))
                    if t >= start and t < end]
        self.time_ids = torch.tensor(time_ids) - start

        guru.info(f"Loading {split} split: {len(self.frame_names)} frames from {start} to {end}")

        # Directory setup
        img_dir_images = f"{data_dir}/images/{factor}x"
        img_dir_rgb = f"{data_dir}/rgb/{factor}x"
        ...
        self.mask_dir = f"{data_dir}/masks/{factor}x"
        if os.path.exists(self.mask_dir) is False:
            self.mask_dir = f"{data_dir}/flow3d_preprocessed/track_anything/{factor}x"
        self.tapip3d_dir = f"{data_dir}/flow3d_preprocessed/anchortap3d"
        self.cache_dir = f"{data_dir}/flow3d_preprocessed/cache"

        # 2D track directory setup
        self.tracks_dir = f"{data_dir}/flow3d_preprocessed/2d_tracks"
        ...
        self.masks_eroded: list[torch.Tensor | None] = [None for _ in self.frame_names]

        # Load pre-computed depths
        depth_path = f"{self.tapip3d_dir}/depth/{factor}x/depths.npy"
        ...
        if depths_all.ndim == 4:
            depths_all = depths_all[:, 0, :, :]
        elif depths_all.ndim != 3:
            raise ValueError(f"Expected depths to be 3D or 4D, got shape {depths_all.shape}")

        self.depths_array = torch.from_numpy(depths_all).float()[start:end]

        # Load camera parameters based on camera_type
        if camera_type == "tapip3d":
            guru.info(f"Loading Tapip3d camera parameters")
            Ks_all = np.load(f"{self.tapip3d_dir}/intrinsic/intrinsics.npy")
            extrinsics_all = np.load(f"{self.tapip3d_dir}/extrinsic/extrinsics.npy")
            ...
            guru.info(f"Loading refined COLMAP camera parameters")
        ...
        elif camera_type == "original":
            guru.info(f"Loading original iPhone camera parameters")
            Ks, w2cs = [], []
        ...
        # Keyframe indices
        tstamps = torch.from_numpy(np.arange(0, len(split_dict["frame_names"])))
        tmask = (tstamps >= start) & (tstamps < end)
        self._keyframe_idcs = tstamps[tmask] - start
        self.scale = 1

        guru.info(f"Loaded {len(self.frame_names)} frames")

        # Scene normalization
        if scene_norm_dict is None:
            cached_scene_norm_dict_path = os.path.join(
                self.cache_dir, f"scene_norm_dict_{split}.pth"
            )
        ...
        # Load covisible masks for validation
        if not self.training and self.has_validation:
            ...
            # Check if directory exists and has the first frame file
            first_frame_path = osp.join(covisible_dir, f"{self.frame_names[0]}.png")
            if os.path.exists(covisible_dir) and os.path.exists(first_frame_path):
                try:
                    self.covisible_masks = (
                        torch.from_numpy(
                            np.array([
                                imageio.imread(
                                    osp.join(covisible_dir, f"{frame_name}.png")
                                )
                    ...
                    # Generate background masks for himor
                    self.bkgd_mask = generate_background_masks(
                        self.covisible_masks.numpy() * 255,
                        self.frame_names
                    )
                    self.bkgd_mask = torch.from_numpy(self.bkgd_mask / 255.0)
                except (FileNotFoundError, OSError) as e:
                    guru.warning(f"Error loading covisible masks from {covisible_dir}: {e}")
                    guru.info("Using all ones (all points covisible)")
                    # Get image shape to create masks with correct dimensions
                    sample_img = self.get_image(0)
                    H, W = sample_img.shape[:2]

                    # Initialize all covisible masks as ones
                    self.covisible_masks = torch.ones(len(self.frame_names), H, W, dtype=torch.float32)
                    self.bkgd_mask = torch.ones(len(self.frame_names), H, W, dtype=torch.float32)
            else:
                guru.info(f"Covisible masks not found at {covisible_dir}, using all ones (all points covisible)")
                # Get image shape to create masks with correct dimensions
                sample_img = self.get_image(0)
                H, W = sample_img.shape[:2]
    ...
    def load_mask(self, index) -> torch.Tensor:
        ...
        if os.path.exists(path):
            ...
            if os.path.exists(path_jpg):
                ...
                mask_dir_1x = self.mask_dir.replace(f"/{self.factor}x", "/1x") if f"/{self.factor}x" in self.mask_dir else f"{self.data_dir}/masks/1x"
    ...
    def load_mask_eroded(self, index) -> torch.Tensor:
        """Load binary eroded mask for 3D track initialization."""
        path = f"{self.mask_dir}/{self.frame_names[index]}.png"
        ...
        # Convert to binary foreground mask
        fg_mask = mask.reshape((*mask.shape[:2], -1)).max(axis=-1) > 0

        # Apply erosion if enabled
        if self.mask_erosion_kernel > 0:
            kernel = np.ones((self.mask_erosion_kernel, self.mask_erosion_kernel), np.uint8)
            fg_mask = cv2.erode(fg_mask.astype(np.uint8), kernel, iterations=1)

        return torch.from_numpy(fg_mask).float()

    def get_mask_eroded(self, index) -> torch.Tensor:
        """Get eroded mask for 3D track initialization."""
        if self.masks_eroded[index] is None:
            self.masks_eroded[index] = self.load_mask_eroded(index)
        mask = cast(torch.Tensor, self.masks_eroded[index])
        return mask
    ...
    def load_target_tracks(
        ...
        for ti in target_indices:
            ...
            path = f"{self.tracks_dir}/{self.factor}x/{q_name}_{t_name}.npy"
            tracks = np.load(path).astype(np.float32)
            tracks[:, :2] = tracks[:, :2] / self.factor
    ...
    def load_tracks_3d(self, query_index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        tracks_full = np.load(track_path).astype(np.float32)
        ...
        tracks_3d = torch.from_numpy(tracks_full[:, :, :3])
        tracks_3d = tracks_3d.permute(1, 0, 2)

        visibility = torch.from_numpy(tracks_full[:, :, 3])
        visibility = visibility.permute(1, 0)
        visibles = visibility > 0.5

        # Compute foreground mask
        query_tracks_3d = tracks_3d[:, query_index, :]
        ...
        # Use eroded mask for filtering if erosion is enabled
        if self.mask_erosion_kernel > 0:
            mask = self.get_mask_eroded(query_index)
            H, W = mask.shape
            ...
            fg_mask = mask[xy_int[:, 1], xy_int[:, 0]] > 0.5
        else:
            # No mask filtering when erosion is disabled
            fg_mask = torch.ones(len(points_2d), dtype=torch.bool)
    ...
    def get_tracks_3d_raw(
        ...
        **kwargs,
        ...
        for q_idx in query_idcs:
            tracks_3d, fg_mask, tracks_visibles = self.load_tracks_3d(q_idx)
            ...
            tracks_3d_homo = F.pad(tracks_3d, (0, 1), value=1.0)
            if hasattr(self, 'transform'):
            ...
            # visible: confidence=1.0, invisible: confidence=0.5
            confidences = tracks_visibles.float() + (~tracks_visibles).float() * 0.5
        ...
        visibles = torch.cat(all_visibles, dim=0)
        invisibles = torch.cat(all_invisibles, dim=0)
    ...
    def _compute_canonical_time(
        ...
        max_visible_count = 0
        best_cano_t = query_idcs[len(query_idcs) // 2]
    ...
    def get_tracks_3d(
        ...
        **kwargs,
        ...
        for q_idx in query_idcs:
            ...
            tracks_3d = tracks_3d[fg_mask]
            tracks_visibles = tracks_visibles[fg_mask]
    ...
    def get_video_dataset(self):
        """Return a dataset view for video rendering."""
        from torch.utils.data import Dataset
        return iPhoneTapip3dDatasetVideoView(self)
    ...
    def __getitem__(self, index: int):
        ...
        data = {
            ...
            "ts": self.time_ids[index] if not self.training else torch.tensor(index),
            "w2cs": self.w2cs[index],
            "Ks": self.Ks[index],
            "imgs": self.get_image(index),
        }

        # Add valid_masks (himor requirement)
        img = self.get_image(index)
        if img.shape[-1] == 4:
            data["valid_masks"] = img[..., 3]
        else:
            data["valid_masks"] = torch.ones(img.shape[:2], dtype=torch.float32)

        if self.training:
            data["depths"] = self.get_depth(index)
        ...
        if self.training:
            ...
            # Validation mode
            if hasattr(self, 'covisible_masks'):
                data["covisible_masks"] = self.covisible_masks[index]
            if hasattr(self, 'bkgd_mask'):
...
class iPhoneTapip3dDatasetVideoView(Dataset):
    """Return a dataset view of the video trajectory."""

    def __init__(self, dataset: iPhoneTapip3dDataset):
        super().__init__()
        self.dataset = dataset
        # Try to get fps, default to 30.0 if not available
        extra_json_path = os.path.join(dataset.data_dir, "extra.json")
        if os.path.exists(extra_json_path):
            with open(extra_json_path) as f:
                extra_dict = json.load(f)
            self.fps = float(extra_dict.get("fps", 30.0))
        else:
            self.fps = 30.0
    ...
    def __getitem__(self, index):
        # Get tri-label mask (-1=background, 0=uncertain, 1=foreground)
        tri_mask = self.dataset.get_mask(index)
        # Convert to binary mask (0 or 1) for video visualization
        binary_mask = (tri_mask == 1).float()
...
def generate_background_masks(mask, val_names, threshold=0.8, kernel_size=(3, 3), erode_iterations=3):
    """
    Generate background masks for validation (himor feature).
    """
