...
class iPhoneDataConfig:
    ...
    mask_erosion_kernel: int = 0  # Kernel size for mask erosion (0=disabled, 3/5/7 recommended)
...
class iPhoneDataset(BaseDataset):
        ...
        **_,
        ...
        if not skip_load_imgs:
            ...
            if self.training:
                ...
                # Handle multi-channel masks by converting to binary mask
                if masks_array.ndim == 4 and masks_array.shape[-1] > 1:
                    # Any pixel with color (any channel > threshold) is considered mask
                    # This handles blue, white, or any colored masks
                    masks_array = (masks_array.max(axis=-1) > 10).astype(np.float32) * 255.0

                # Store original masks
                self.masks = torch.from_numpy(masks_array) / 255.0

                # Create eroded masks for 3D track initialization if erosion is enabled
                if self.mask_erosion_kernel > 0:
                    kernel = np.ones((self.mask_erosion_kernel, self.mask_erosion_kernel), np.uint8)
                    masks_array_eroded = np.zeros_like(masks_array)
                    for i in range(len(masks_array)):
                        masks_array_eroded[i] = cv2.erode(masks_array[i], kernel, iterations=1)
                    self.masks_eroded = torch.from_numpy(masks_array_eroded) / 255.0
                    guru.info(f"Created eroded masks with kernel size {self.mask_erosion_kernel} for 3D track query frame filtering")
                def load_depth(frame_name):
                    ...
                    if self.depth_type == "depth_anything_lidar":
                        depth = np.load(
                            osp.join(
                                self.data_dir,
                ...
                if osp.exists(covisible_dir) and osp.exists(first_frame_path):
                    try:
                ...
                if osp.exists(osp.join(self.data_dir, "flow3d_preprocessed/track_anything/",f"{factor}x/{self.frame_names[0]}.png",)):
                    ...
                    if self.mask_erosion_kernel > 0:
                        ...
                        for i in range(len(masks_array)):
                            masks_array_eroded[i] = cv2.erode(masks_array[i], kernel, iterations=1)
                        self.masks_eroded = torch.from_numpy(masks_array_eroded) / 255.0
                        guru.info(f"Created eroded masks with kernel size {self.mask_erosion_kernel} for 3D track query frame filtering (validation)")

                # Generate background masks only if covisible masks were successfully loaded
                try:
                    ...
                    guru.warning(f"Error generating background masks: {e}")
                    guru.info("Using all ones for background masks")
                    H, W = self.imgs.shape[1:3]
    ...
    def get_tracks_3d(
        ...
        for i, tracks_2d in enumerate(raw_tracks_2d):
            ...
            # Handle query frame filtering based on erosion settings
            if self.mask_erosion_kernel > 0:
                # Use eroded mask for query frame filtering
                query_frame_idx = candidate_frames[i]
                eroded_mask = self.masks_eroded[query_frame_idx] * self.valid_masks[query_frame_idx] * (self.depths[query_frame_idx] > 0)
                eroded_mask = (eroded_mask > 0.5).float()
                is_in_query_mask = (
                    F.grid_sample(
                        eroded_mask[None, None],
                        normalize_coords(tracks_2d[i:i+1, :, None, :], H, W),
                        align_corners=True,
                    ).squeeze()
                    == 1
                )
                is_in_masks[i] = is_in_query_mask
            else:
                # Don't filter on query frame when erosion is disabled
                is_in_masks[i] = True
            ...
            # Skip if no valid tracks after mask filtering
            if len(visible_counts) == 0 or visible_counts.sum() == 0:
                guru.info(f"PASS: Query frame {candidate_frames[i]} (index {i}) - No valid tracks after mask filtering (total tracks before filtering: {tracks_2d.shape[1]})")
                continue
        ...
        if len(filtered_tracks_3d) == 0:
            raise ValueError(
                f"No valid tracks found after mask filtering with erosion kernel={self.mask_erosion_kernel}. "
                f"Try reducing the erosion kernel size or check your masks."
            )
        ...
        guru.info(f"Filtered tracks: {filtered_tracks_3d.shape[0]} tracks from {len(raw_tracks_2d)} query frames")
        if step == 1:
            torch.save(
                {
